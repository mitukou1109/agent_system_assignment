...
class Go2Controller : public SimpleController
{
  Body* ioBody;
  double dt;
 ...
 public:
  virtual bool initialize(SimpleControllerIO* io) override
  {
    ...
    // find the cfgs file
    fs::path inference_target_path
        = fs::path(std::getenv("HOME"))
          / fs::path("agent-system/logs/go2-walking/inference_target");
    ...
    resample_interval_steps = static_cast<int>(
        std::round(env_cfg->get("resampling_time_s", 4.0) / dt));
    ...
    lin_vel_x_range = Vector2(range_listing->at(0)->toDouble(),
                              range_listing->at(1)->toDouble());
    range_listing = command_cfg->findListing("lin_vel_y_range");
    lin_vel_y_range = Vector2(range_listing->at(0)->toDouble(),
                              range_listing->at(1)->toDouble());
    range_listing = command_cfg->findListing("ang_vel_range");
    ang_vel_range = Vector2(range_listing->at(0)->toDouble(),
                            range_listing->at(1)->toDouble());
    ...
    dist_lin_x = std::uniform_real_distribution<double>(lin_vel_x_range[0],
                                                        lin_vel_x_range[1]);
    dist_lin_y = std::uniform_real_distribution<double>(lin_vel_y_range[0],
                                                        lin_vel_y_range[1]);
    dist_ang = std::uniform_real_distribution<double>(ang_vel_range[0],
                                                      ang_vel_range[1]);
  ...
  bool inference(VectorXd& target_dof_pos, const Vector3d& angular_velocity,
                 const Vector3d& projected_gravity, const VectorXd& joint_pos,
                 const VectorXd& joint_vel)
  {
    try
    {
      ...
      // auto input = torch::from_blob(obs_vec.data(), {1,
      // (long)obs_vec.size()}, torch::kFloat32).to(torch::kCUDA);
      auto input = torch::from_blob(obs_vec.data(), {1, (long)obs_vec.size()},
                                    torch::kFloat32)
                       .to(torch::kCPU);
  ...
  virtual bool control() override
  {
    ...
    Vector3 projected_gravity
        = root_coord.linear().transpose() * global_gravity;
    ...
    {
      inference(target_dof_pos, angular_velocity, projected_gravity, joint_pos,
                joint_vel);
    ...
    for (int i = 0; i < num_actions; ++i)
    {
      ...
      double q = joint->q();
      double dq = joint->dq();
      // double u = P_gain * (target_dof_pos[i] - q) + D_gain *
      // (target_dof_vel[i] - dq);
...
CNOID_IMPLEMENT_SIMPLE_CONTROLLER_FACTORY(Go2Controller)
